...
const IndicatorModal: React.FC<IndicatorModalProps> = ({ isOpen, onClose, data }) => {
    const serieData = data.serie;

    const renderItem = ({ item }: { item: SerieItem }) => (
        <View style={styles.row}>
            <Text style={styles.cell}><Text style={styles.label}>Fecha:</Text> {item.fecha}</Text>
            <Text style={styles.cell}><Text style={styles.label}>Valor:</Text> {item.valor.toFixed(2)}</Text>
        </View>
    );
